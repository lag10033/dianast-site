...
function calcItem(el, k) {
  ...
  if (mode === 'meter') {
    ...
    if (isList) {
      ...
      // для общего заказа лист идёт в м²: площадь × цена м² из прайса
      const s1 = wM * unitLen;                        // м² одного куска
      листМ2 = r3(s1 * qty);                          // общая площадь, м²
      ставкаМ2 = (s1 > 0 && r2(m2 * s1) < minB)       // минималка задрала цену куска?
        ? r2(perPiece / s1)                           // да → эффективная цена/м²
        : r2(m2 * kShort * k);                        // нет → прайсовая цена/м²
    } else {
      const mpM = r2(Math.max(r2(m2 * wM), minB) * kShort * k);   // руб/м.п.
  ...
  return { наимен, ед, колво, колвоСтр, цена, сумма, ready, invalid: !!warn, раскрой,
           лист: isList, площадьМ2: листМ2, ставкаМ2,
